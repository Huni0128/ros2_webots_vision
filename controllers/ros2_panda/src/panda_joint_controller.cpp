...
PandaJointController::PandaJointController(webots::Robot* robot, const rclcpp::Node::SharedPtr& node, int timestep)
  : robot_(robot), node_(node), timestep_(timestep) {
  ...
  RCLCPP_INFO(node_->get_logger(), "PandaJointController initialized with %zu joints.", joints_.size());

  // 관절 상태 퍼블리셔 설정
  joint_pub_ = node_->create_publisher<sensor_msgs::msg::JointState>("/panda_joint_states", 10);

  // 관절 제어 명령 서브스크라이버 설정
  joint_sub_ = node_->create_subscription<std_msgs::msg::Float64MultiArray>(
    "/panda_joint_command", 10,
    std::bind(&PandaJointController::jointCommandCallback, this, std::placeholders::_1)
  );
}
...
void PandaJointController::jointCommandCallback(const std_msgs::msg::Float64MultiArray::SharedPtr msg) {
  if (msg->data.size() != joints_.size()) {
    RCLCPP_WARN(node_->get_logger(), "Invalid joint command size: %zu (expected %zu)", msg->data.size(), joints_.size());
    return;
  }
